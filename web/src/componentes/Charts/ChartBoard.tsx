            ...
            <div className={`grid grid-cols-4 col-span-2 gap-3`}>
                <div className="space-y-3">
                    <div className={`${showInsights || 'hidden'}`}>
                        {cpuChart && <CpuChart chartMode="minimalist" onChartClick={(data) => { updateFocusChart(data, setFocusChartData) }} />}
                    </div>
                    <div className={`${showInsights || 'hidden'}`}>
                        {transferChart && <TransferChart chartMode="minimalist" onChartClick={(data) => { updateFocusChart(data, setFocusChartData) }}  />}
                    </div>
                    <div className={`${showInsights || 'hidden'}`}>
                        {transferChart && <MemoryChart chartMode="minimalist" onChartClick={(data) => { updateFocusChart(data, setFocusChartData) }} />}
                    </div>
                </div>
